...
public class UtilisateurDAO {
    ...
    public int countByRole(String role) {
        try (Connection conn = db.getConnection(); PreparedStatement ps = conn.prepareStatement(
            ...
            ps.setString(1, role);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getInt(1); // ← CORRECTION : rs.next() ajouté
        } catch (Exception e) { e.printStackTrace(); }
        return 0;
    }
    ...
    private Utilisateur buildUser(ResultSet rs, Connection conn) throws SQLException {
        ...
        switch (role) {
            ...
            case "GESTIONNAIRE":
                return new Gestionnaire(id, nom, prenom, email, mdp);
            case "ENSEIGNANT":
                return new Enseignant(id, nom, prenom, email, mdp);
            case "ETUDIANT":
                String ine = "";
                String niveau = "L1";
                try (PreparedStatement ps2 = conn.prepareStatement(
                    ...
                    if (rs2.next()) {
                        ine    = rs2.getString("INE")    != null ? rs2.getString("INE")    : "";
                        niveau = rs2.getString("niveau") != null ? rs2.getString("niveau") : "L1";
                    }
                } catch (Exception e) { e.printStackTrace(); }
