...
/**
 * Service d'envoi d'emails. Par défaut en mode simulation (affiche dans la console).
 * Pour activer les vrais emails, créez un fichier email.properties dans le répertoire courant :
 *   smtp.host=smtp.gmail.com
 *   smtp.port=587
 *   smtp.user=votre@email.com
 *   smtp.password=votremotdepasse
 *   smtp.enabled=true
 */
public class EmailService {
    ...
    static {
        try {
            ...
            if (f.exists()) {
                ...
                simulationMode = !"true".equalsIgnoreCase(p.getProperty("smtp.enabled", "false"));
            }
    ...
    public static void sendNotification(Utilisateur user, String subject, String body) {
        ...
        if (simulationMode) {
            ...
            System.out.println("│ Corps   : " + body.replace("\n", "\n│          "));
            System.out.println("└──────────────────────────────────────────────────────");
            return;
        }
    ...
    public static void notifierValidationReservation(Utilisateur user, Reservation r) {
        ...
        String body = "Bonjour " + user.getNomComplet() + ",\n\n"
            ...
            + "  • Motif    : " + r.getMotif() + "\n"
            + "  • Date     : " + (r.getDateReservation() != null ? r.getDateReservation().toLocalDate() : "N/A") + "\n\n"
            + "Cordialement,\nUNIV-SCHEDULER";
    ...
    public static void notifierRefusReservation(Utilisateur user, Reservation r) {
        ...
        String body = "Bonjour " + user.getNomComplet() + ",\n\n"
            + "Votre réservation de la salle " + r.getSalleNumero() + " a été refusée.\n"
    ...
    public static void notifierChangementSalle(Utilisateur user, Cours cours, String ancienneSalle) {
    ...
    public static void envoyerRappelFinReservation(Utilisateur user, Reservation r) {
        ...
        String body = "Bonjour " + user.getNomComplet() + ",\n\n"
            + "Votre réservation de la salle " + r.getSalleNumero() + " se termine bientôt.\n"
            + "  • Motif  : " + r.getMotif() + "\n"
            + "  • Heure  : " + (r.getDateReservation() != null ? r.getDateReservation().toLocalTime() : "N/A") + "\n\n"
    ...
    private static String buildHtmlEmail(String subject, String body, String nomComplet) {
        String bodyHtml = body.replace("\n", "<br>").replace("  •", "&nbsp;&nbsp;•");
        return "<!DOCTYPE html><html><body style='font-family:Segoe UI,Arial,sans-serif;background:#f0f4f8;margin:0;padding:20px;'>"
            + "<div style='max-width:520px;margin:auto;background:white;border-radius:12px;overflow:hidden;"
            + "box-shadow:0 4px 16px rgba(0,0,0,0.1);'>"
            + "<div style='background:#1e293b;padding:24px 32px;'>"
            + "<div style='color:#f8fafc;font-size:20px;font-weight:bold;'>🎓 UNIV-SCHEDULER</div>"
            + "<div style='color:#94a3b8;font-size:13px;margin-top:4px;'>" + subject + "</div>"
            + "</div>"
            + "<div style='padding:32px;color:#1e293b;font-size:14px;line-height:1.7;'>"
            + bodyHtml
            + "</div>"
            + "<div style='background:#f8fafc;padding:16px 32px;color:#64748b;font-size:11px;border-top:1px solid #e2e8f0;'>"
            + "Cet email a été envoyé automatiquement par UNIV-SCHEDULER. Ne pas répondre à cet email."
            + "</div></div></body></html>";
    }
