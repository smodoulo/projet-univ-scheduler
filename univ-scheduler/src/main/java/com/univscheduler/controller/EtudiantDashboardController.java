...
public class EtudiantDashboardController extends BaseController {
    ...
    protected void onUserLoaded() {
        ...
        // ✅ Présélectionner la vraie classe de l'étudiant connecté
        ClassePedago classeEtudiant = null;
        if (currentUser instanceof Etudiant) {
            Etudiant etu = (Etudiant) currentUser;
            classeEtudiant = classes.stream()
                    .filter(c -> c.getId() == etu.getClasseId())
                    .findFirst()
                    .orElse(null);
        }
        if (classeEtudiant == null && !classes.isEmpty())
            classeEtudiant = classes.get(0);
    ...
    private void buildProfil() {
        ...
        if (profilEmail != null) profilEmail.setText(
                currentUser.getEmail() != null ? currentUser.getEmail() : "—");

        if (currentUser instanceof Etudiant) {
            Etudiant etu = (Etudiant) currentUser;
            if (profilINE    != null) profilINE.setText(
                    etu.getINE() != null && !etu.getINE().isEmpty() ? etu.getINE() : "—");
            if (profilNiveau != null) profilNiveau.setText(
                    etu.getNiveau()   != null ? etu.getNiveau()   : "—"); // ✅ "L3"
            if (profilClasse != null) profilClasse.setText(
                    etu.getClasseNom() != null ? etu.getClasseNom() : "—"); // ✅ "CHIMIE-L3"
        } else {
            if (profilINE    != null) profilINE.setText("—");
            if (profilNiveau != null) profilNiveau.setText("—");
            if (profilClasse != null) profilClasse.setText("—");
    ...
    private void buildStats() {
        ...
        long nbMat = coursList.stream()
                .map(Cours::getMatiereNom).filter(Objects::nonNull).distinct().count();
        ...
        // ✅ Niveau depuis l'étudiant connecté — pas depuis classeCombo
        if (statNiveauLabel != null) {
            String niveau = "—";
            if (currentUser instanceof Etudiant)
                niveau = ((Etudiant) currentUser).getNiveau();
            if (niveau == null || niveau.isBlank()) niveau = "—";
            statNiveauLabel.setText(niveau); // ✅ affiche "L3" au lieu de "L1"
        }
    }
    ...
    private void buildChart() {
        ...
        chart.setTitle("📚 Cours par Matière");
    ...
    private void buildCalendar() {
        ...
        corner.setStyle("-fx-font-weight:bold;-fx-background-color:#0f172a;-fx-text-fill:white;"
                + "-fx-padding:8;-fx-alignment:CENTER;");
        ...
        for (int j = 0; j < jours.length; j++) {
            ...
            lbl.setStyle("-fx-font-weight:bold;-fx-background-color:#0f172a;-fx-text-fill:white;"
                    + "-fx-padding:8;-fx-alignment:CENTER;-fx-font-size:11px;");
        ...
        for (Cours c : coursList)
            if (c.getCreneauInfo() != null)
                for (String j : jours)
                    if (c.getCreneauInfo().toUpperCase().startsWith(j.toUpperCase())) {
                        par.get(j).add(c); break;
                    }

        for (int h = 0; h < heures.length; h++) {
            Label hLbl = new Label(heures[h] + "h");
            hLbl.setStyle("-fx-background-color:#f1f5f9;-fx-font-weight:bold;"
                    + "-fx-padding:8 4;-fx-alignment:CENTER;-fx-font-size:11px;");
            hLbl.setMaxWidth(Double.MAX_VALUE);
            hLbl.setMaxHeight(Double.MAX_VALUE);
            ...
            for (int j = 0; j < jours.length; j++) {
                ...
                for (Cours c : par.get(jours[j])) {
                    if (c.getCreneauInfo() != null && c.getCreneauInfo().contains(heures[h] + "h")) {
                        ...
                        m.setStyle("-fx-background-color:#0ea5e9;-fx-text-fill:white;"
                                + "-fx-padding:2 6;-fx-background-radius:4;-fx-font-size:10px;");
                        m.setWrapText(true);
                        Label horaire = new Label("⏰ " + hD + " → " + hF);
                        horaire.setStyle("-fx-text-fill:#0ea5e9;-fx-font-size:9px;-fx-font-weight:bold;");
                        Label ens = new Label("👤 " + (c.getEnseignantNom() != null ? c.getEnseignantNom() : "?"));
                        ens.setStyle("-fx-text-fill:#64748b;-fx-font-size:9px;");
                        Label sal = new Label("🏫 " + (c.getSalleNumero() != null ? c.getSalleNumero() : "?"));
                        sal.setStyle("-fx-text-fill:#64748b;-fx-font-size:9px;");
                        cell.getChildren().addAll(m, horaire, ens, sal);
                    }
                }
        ...
        if (calendarWeekLabel != null)
            calendarWeekLabel.setText("Semaine du " + calendarWeekStart.getDayOfMonth()
                    + "/" + calendarWeekStart.getMonthValue()
                    + "/" + calendarWeekStart.getYear());
    }
    ...
    private void setupNotifTable() {
        ...
        if (colNotifMsg   != null) colNotifMsg.setCellValueFactory(d ->
                new SimpleStringProperty(d.getValue().getMessage()));
        if (colNotifType  != null) colNotifType.setCellValueFactory(d ->
                new SimpleStringProperty(d.getValue().getType()));
        if (colNotifDate  != null) colNotifDate.setCellValueFactory(d ->
                new SimpleStringProperty(d.getValue().getDateEnvoi() != null
                        ? d.getValue().getDateEnvoi().toLocalDate().toString() : ""));
    ...
    @FXML private void handleRechercherSalles() {
        ...
        if (result.isEmpty()) {
            ...
            resultLabel.setText("✅ " + result.size() + " salle(s) disponible(s)" + eq
                    + (label.isEmpty() ? "" : " @ " + label));
